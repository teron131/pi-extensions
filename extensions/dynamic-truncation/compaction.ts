...
const parseCompactionRequest = (args: string): CompactionRequest => {
	...
	return {
		...
		modelId,
		customInstructions:
			customInstructions.length > 0 ? customInstructions.join(" ") : undefined,
	};
};
...
export function registerCompactionHooks(pi: ExtensionAPI): void {
	...
	const triggerCompaction = (
		...
		try {
			void ctx.compact({
				...
				onError: (error) => {
					settleCompaction(ctx, `Compaction failed: ${error.message}`, "error");
				},
			});
		} catch (error) {
	...
	pi.on("session_before_compact", async (event, ctx) => {
		...
		if (ctx.hasUI && !pendingCompactionRequest) {
			ctx.ui.setStatus(
				COMMAND_NAME,
				ctx.ui.theme.fg("muted", `⚙ compacting ${allMessages.length} msgs...`),
			);
		}
		...
		try {
			...
			const formattedFiles = formatFileOperations(readFiles, modifiedFiles);
