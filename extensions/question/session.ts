	...
	return ctx.ui.custom<QuestionnaireResult>((tui, theme, _kb, done) => {
		...
		editor.onSubmit = (value) => {
			...
			if (inputMode === "customChoice") {
				...
				stopEditor();
				startEditor("note", question, answers.get(question.id)?.note ?? "");
				return;
			}
		...
		function handleInput(data: string) {
			...
			if (isMulti) {
				if (matchesKey(data, Key.tab) || matchesKey(data, Key.right)) {
					currentTab = (currentTab + 1) % (visibleQuestions.length + 1);
					refresh();
					return;
				}
			...
			if (matchesKey(data, Key.up)) {
				setSelectionIndex(question.id, Math.max(0, currentSelection - 1));
				refresh();
				return;
			}
			...
			if (matchesKey(data, Key.enter)) {
				...
				if (selected.kind === "option" && selected.option) {
					commitOptionChoice(question, selected.option, currentSelection);
					advance();
				}
			}
		}
		...
		function render(width: number): string[] {
			...
			if (inputMode && question) {
				...
				lines.push("");
				addWrapped(theme.fg("muted", `${question.noteLabel}:`), " ", " ");
				...
				for (const visibleQuestion of visibleQuestions) {
					...
					for (const line of formatAnswerSummary(visibleQuestion, answer).split(
						"\n",
					)) {
