	...
	const syncTodoUi = (ctx: ExtensionContext) => {
		...
		ctx.ui.setWidget(TODO_WIDGET_ID, (_tui, theme) => ({
			render(width: number): string[] {
				return renderTodoWidgetLines(getTodos(), theme, width, widgetExpanded);
			},
			invalidate() {},
		}));
	};
		...
		async execute(
			...
			switch (params.action) {
				...
				case "add": {
					...
					const baseTodos = restartedFromCompletedList ? [] : currentTodos;
				...
				case "remove": {
					...
					if (result.error || !result.targets?.length) {
						return buildErrorResult("remove", result.error ?? "item not found");
					}

					const removedIds = new Set(
						result.targets.map((target) => target.todo.id),
					);
					commitTodos(currentTodos.filter((todo) => !removedIds.has(todo.id)));
					return {
						content: [
							{
								type: "text",
				...
				case "toggle": {
					...
					if (target.error || !target.todo) {
						return buildErrorResult("toggle", target.error ?? "item not found");
					}
						...
						content: [
							{
								...
								text: formatToggleTodoMessage(
									target.todo,
									formatTodoTargetLabel(target.todo, target.position),
								),
							},
						],
				...
				case "note": {
					const target = findTodoTarget(currentTodos, params);
					if (target.error || !target.todo) {
						return buildErrorResult("note", target.error ?? "item not found");
					}
				...
				case "reset": {
					...
					return {
						content: [
							{
								type: "text",
								text: formatResetTodoMessage(params.action, count),
							},
						],
				...
				default:
					return {
						...
						details: buildDetails("list", `unknown action: ${params.action}`),
					};
			}
		},
