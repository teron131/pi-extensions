		...
		ctx.ui.setFooter((tui, theme, footerData) => {
			...
			return {
				...
				render(width: number): string[] {
					...
					let modelSide = ctx.model ? `🤖${ctx.model.id}` : "🤖no-model";
					...
					for (
						...
						if (entry.type === "message") {
							const messagePayload = entry.message as MessagePayload;
							if (!lastMsgEntry) lastMsgEntry = entry;
							if (messagePayload.role === "user" && !lastUserEntry) {
								lastUserEntry = entry;
							}
						}
					}
					...
					if (lastUserEntry && lastMsgEntry) {
						const userMessage = lastUserEntry.message as MessagePayload;
						const startTime =
							userMessage.timestamp ??
							new Date(lastUserEntry.timestamp).getTime();

						const lastMessage = lastMsgEntry.message as MessagePayload;
						isRunning =
						...
						if (isRunning) {
							currentRunTimeMs = Math.max(0, Date.now() - startTime);
							sessionRunTimes.set(lastMsgEntry.id, currentRunTimeMs);
						} else if (sessionRunTimes.has(lastMsgEntry.id)) {
							currentRunTimeMs = sessionRunTimes.get(lastMsgEntry.id) ?? 0;
						} else {
							const endTime =
					...
					if (gitDiffCounts) {
						sessionParts.unshift(formatDiffSummary(theme, gitDiffCounts, " "));
					}
					...
					return renderFooterBlockLines(width, {
						...
						bottomRight: statusSide ? theme.fg("dim", statusSide) : "",
					});
				},
			};
		});
